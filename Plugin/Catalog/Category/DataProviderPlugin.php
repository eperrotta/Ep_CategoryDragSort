...
use Ep\CategoryDragSort\Model\ResourceModel\Category\Product\Position as ProductPositionResource;
...
class DataProviderPlugin
{
    ...
    public function aroundGetData(CategoryDataProvider $dataProvider, \Closure $proceed): array
    {
        $data = $proceed();
        ...
        if ($currentCategory->getStoreId()) {
            ...
            $data[$currentCategory->getId()]['default']['sorted_products'] = $this->getProductSavedPositions(
                $globalCategory
            );
        }
    ...
    private function getProductSorterLoadUrl(Category $category)
    {
        ...
        if ($storeId) {
            ...
            $url = $this->urlBuilder->getUrl('categorydragsort/category_virtual/preview', $urlParams);
        }

        return $url;
    }

    private function getProductSavedPositions(Category $category): string
    {
        $productPositions = [];
    ...
    private function getDefaultStoreId()
    {
        ...
        if (null === $store) {
            ...
            $store = !empty($this->storeManager->getWebsites()) ? current(
